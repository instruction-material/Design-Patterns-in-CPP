...
class Command {
  public:
    ...
    virtual bool isReplaceCommand() const {
        return false;
    }
};
...
class ReplaceCommand final : public Command {
  public:
    ...
    bool isReplaceCommand() const override {
        return true;
    }
...
class DraftMode final : public Mode {
  public:
    ...
    bool allowReplace() const override {
        return true;
    }
};
...
class EditorSession {
  public:
    ...
    void setMode(std::unique_ptr<Mode> nextMode) {
        mode_ = std::move(nextMode);
    }

    void execute(const Command& command) {
        if (command.isReplaceCommand() && !mode_->allowReplace()) {
            std::cout << "Mode " << mode_->name() << " rejected command "
                      << command.label() << '\n';
            return;
        }
        ...
        std::cout << "Applied " << command.label() << " in mode "
                  << mode_->name() << '\n';
    }
