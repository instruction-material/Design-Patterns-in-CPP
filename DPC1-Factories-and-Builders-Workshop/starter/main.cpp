...
class LightThemeFactory final : public ThemeFactory {
  public:
    std::string accent() const override {
        return "sky";
    }
    std::string surface() const override {
        return "paper";
    }
};
...
class DashboardBuilder {
  public:
    ...
    DashboardBuilder& addMetricCard(const std::string& label,
                                    const std::string& value) {
        widgets_.push_back(
            std::make_unique<MetricCard>(label, value, theme_.accent()));
        return *this;
    }

    DashboardBuilder& addActionButton(const std::string& title) {
        widgets_.push_back(
            std::make_unique<ActionButton>(title, theme_.surface()));
        return *this;
    }
...
int main() {
    ...
    builder.addMetricCard("Active users", "184")
        .addMetricCard("Build queue", "9")
        .addActionButton("Flush cache");

    std::cout << "Starter dashboard\n";
    builder.render(std::cout);
    std::cout << "\nTODO: add a second factory family or a richer builder step "
                 "when the UI must vary by environment.\n";
}
