...
template <typename ShippingPolicy> class ShippingCalculator {
  public:
...
void printScenario(const DiscountStrategy& strategy, const Order& order) {
    ...
    std::cout << strategy.name() << " discount saves $" << std::fixed
              << std::setprecision(2) << savings << '\n';
}
...
int main() {
    ...
    LambdaDiscount flashSale("flash-sale", [](const Order& currentOrder) {
        return currentOrder.daysEarly <= 1 ? currentOrder.cartTotal * 0.08
                                           : 0.0;
    });
    ...
    std::cout << "\nThe runtime strategy chooses discount behavior; the "
                 "template policy chooses shipping behavior at compile time.\n";
}
