...
class LegacyCsvAdapter {
  public:
    explicit LegacyCsvAdapter(LegacyCsvLoader loader)
...
class ImportFacade {
  public:
    explicit ImportFacade(LegacyCsvAdapter csvAdapter)
        : csvAdapter_(std::move(csvAdapter)) {
    }
...
int main() {
    ...
    std::cout << "\nTODO: add a second adapter so the facade can hide both CSV "
                 "and another external format.\n";
}
